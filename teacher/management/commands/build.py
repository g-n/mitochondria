...
class Command(BaseCommand):
    ...
    def __init__(self):
        self.first_names = open('teacher/fixtures/first_names.txt', 'r').read().splitlines()
        self.last_names = open('teacher/fixtures/last_names.txt', 'r').read().splitlines()
        super().__init__()
    ...
    def make_classroom(self, user, name, problem_sets, num_students=10):
        ...
        for _ in range(num_students):
            jimmy = Student.objects.create(first_name=random.choice(self.first_names), last_name=random.choice(self.last_names), classroom=c)
            jimmy.save()
    ...
    def gen_problem(self, lim, min=0):
        ...
        return {'target' :"{}+_={}".format(str(num1), str(answer)), 'correct':str(correct), 'incorrect' :str(incorrect)}
    ...
    def handle(self, *args, **options):
        ...
        easy_addition.save()
        for _ in range(20):
            Problem.objects.create(set_name=easy_addition, **self.gen_problem(10)).save()

        big_number = ProblemSet.objects.create(set_name="large numbers", user=u1)
        easy_addition.save()
        for _ in range(15):
            Problem.objects.create(set_name=easy_addition, **self.gen_problem(500, 100)).save()
